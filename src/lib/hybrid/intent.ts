...
const rules: PatternRule[] = [
  ...
  // Recipe search patterns
  {
    ...
    patterns: [
      /recipe\s+for\b/i,
      /how\s+(?:to|do\s+(?:I|you))\s+(?:make|cook|prepare)\b(?!.*(?:less|more|reduce|without|instead|substitute|too\s))/i,
      /(?:make|cook)\s+(?:me\s+)?(?:a\s+|some\s+)?(?!.*(?:less|more|healthier|spicier|milder))(.+)/i,
      /I\s+want\s+(?:to\s+(?:make|cook|eat)|some)\b(?!.*(?:less|more|reduce|without))/i,
      /suggest\s+(?:a\s+|some\s+)?recipe/i,
      /find\s+(?:me\s+)?(?:a\s+)?recipe/i,
      /show\s+(?:me\s+)?(?:a\s+)?recipe/i,
      /what\s+(?:can|should)\s+I\s+(?:make|cook)\b(?!.*(?:less|more|instead))/i,
      /looking\s+for\s+(?:a\s+)?(.+)\s+recipe/i,
      /surprise\s+me/i,
      /(?:quick|easy|healthy|simple)\s+(?:dinner|lunch|breakfast|meal|recipe)/i,
      /(?:chicken|beef|lamb|fish|tofu|paneer|shrimp)\s+(?:curry|stir.?fry|stew|soup|salad|pasta|rice)\b(?!.*(?:less|more|too\s|reduce))/i,
    ],
    entityExtractors: {
